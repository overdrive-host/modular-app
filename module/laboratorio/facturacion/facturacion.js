            ...
            try {
                ...
                if (ingresarBtn) {
                    ingresarBtn.addEventListener('click', async (e) => {
                        ...
                        if (isDuplicate) {
                            const shouldProceed = await isDuplicate;
                            if (shouldProceed) {
                                await proceedWithRegistration();
                            }
                        }
                ...
                if (exportExcelBtn) {
                    exportExcelBtn.addEventListener('click', () => {
                        ...
                        const data = facturas.map(factura => ({
                            ...
                            'Monto': formatCurrency(factura.montoFactura, true),
                            'OC': factura.oc,
                    ...
                    importFileInput.addEventListener('change', async (e) => {
                        ...
                        try {
                            ...
                            for (const row of jsonData) {
                                ...
                                const fechaIngreso = row['Fecha de Ingreso'] ? parseDate(row['Fecha de Ingreso']) : null;
                                ...
                                if (!row['Número de Factura'] || !fechaFactura || !parseCurrency(row['Monto']) || !row['OC']) {
...
function openEditModal(factura) {
    ...
    if (editMontoFacturaInput) editMontoFacturaInput.value = formatCurrency(factura.montoFactura, true);
