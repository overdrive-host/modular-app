...
try {
    ...
    function formatDateOnly(date) {
        ...
        if (typeof date === 'string') {
            // Asegurar que la fecha string se parsea como fecha local
            const [year, month, day] = date.split('-');
            parsedDate = new Date(year, month - 1, day); // Meses en JS son 0-indexados
        ...
        // Ajustar a la medianoche local para evitar desfase de zona horaria
    ...
    async function loadAsignaciones() {
        try {
            ...
            const q = query(asignacionesCollection, orderBy('fechaCreacion', 'desc'));
    ...
    async function traspasarAsignaciones(asignacionIds) {
        try {
                ...
                if (asignacionSnap.exists()) {
                    ...
                    if (typeof data.fechaCX === 'string' && data.fechaCX) {
                        ...
                        if (year && month && day && year.length === 4 && month.length === 2 && day.length === 2) {
                            ...
                            fechaCX.setHours(0, 0, 0, 0); // Ajustar a medianoche local
                        } else {
            ...
            // Resto del código de creación de cargos y lotes permanece igual
        ...
        fieldsToClear.forEach(field => {
            ...
            if (field.tagName === 'INPUT') {
                if (field.id.includes('fecha-ingreso')) {
                    field.value = today; // Mantener fecha actual
                    ...
                    if (registrarBtn) {
                        registrarBtn.addEventListener('click', async () => {
                            ...
                            try {
                                ...
                                // Convertir fechaIngreso a Timestamp ajustado a medianoche local
                                const [year, month, day] = fechaIngreso.split('-');
                                const fechaIngresoDate = new Date(year, month - 1, day);
                                fechaIngresoDate.setHours(0, 0, 0, 0); // Ajustar a medianoche local
