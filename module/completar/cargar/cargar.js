...
try {
    ...
    function validateDOMElements() {
        ...
        if (missingElements) {
            console.error('Faltan elementos en el formulario o tablas. Revisa el HTML.');
            return null;
        }
    ...
    async function loadImplanteByAdmision(admision, user) {
        try {
            ...
            console.error('Error al buscar datos de admisión:', error);
    ...
    async function checkPaqueteAndFillFields(codigo) {
        try {
            ...
            console.error('Error al verificar el código en paquetes:', error);
    ...
    async function fetchTotalCotizacionByAdmisionAndProveedor(admision, proveedor, user) {
        try {
            ...
            console.error('Error al buscar total cotización:', error);
            showMessage('Error al buscar total cotización.', 'error');
            return null;
        } finally {
            hideLoading();
        }
    ...
    async function updateReferenciasWithLowerCase() {
        try {
            ...
            showMessage('Error al actualizar documentos.', 'error');
        } finally {
            hideLoading();
        }
    }
    ...
    async function saveChanges() {
        ...
        try {
            ...
            const fechaIngreso = parseDateForTimestamp(ingreso);
            if (!fechaIngreso) {
                showMessage('Fecha de ingreso inválida.', 'error');
                return;
            }
            const cargoData = {
                admision,
            ...
            showMessage('Error al actualizar cargo.', 'error');
        } finally {
            hideLoading();
        }
    }
    ...
    async function loadCargos(user) {
        ...
        try {
            ...
            showMessage('Error al cargar cargos.', 'error');
            return [];
        } finally {
            hideLoading();
        }
    ...
    function showLotesModal(cargoIds) {
        ...
        confirmarLotesBtn.onclick = async () => {
            ...
            try {
                ...
                showMessage('Error al actualizar lotes.', 'error');
            } finally {
                hideLoading();
            }
        };
    ...
    async function downloadExcel(cargos, cargoIds) {
        try {
            ...
            console.error('Error al generar Excel o actualizar Firestore:', error);
        ...
        tabla1Body.querySelectorAll('.fa-edit').forEach(icon => {
            icon.addEventListener('click', async () => {
                ...
                if (cargo) {
                    ...
                    precioInput.value = cargo.precio || '';
    ...
    referenciaInput.addEventListener('change', async () => {
        ...
        if (referencia) {
            ...
            if (refData) {
                precioInput.value = refData.precio || '';
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showMessage('Error al registrar cargo.', 'error');
        } finally {
            hideLoading();
        }
    });
    ...
    showMessage('Error en la inicialización.', 'error');
}