...
try {
    ...
    let initialTableWidth = 0; // Variable para almacenar el ancho inicial de la tabla
    ...
    // Cargar registros de ambas colecciones
    async function loadRecords() {
        const user = auth.currentUser;
        if (!user) throw new Error('Usuario no autenticado');

        // Cargar pacientesimplantes
        ...
        const consignaciones = consignacionSnapshot.docs.map(doc => ({
            ...
            fuente: 'Consignación',
            ...doc.data(),
            atributo: doc.data().modalidad // Mapear 'modalidad' a 'atributo' para consistencia
        }));

        // Combinar y estandarizar registros
    ...
    function filterRecords(records, year, month, estado) {
        ...
        return records.filter(record => {
            ...
            return Object.entries(columnFilters).every(([index, filterValue]) => {
                ...
                const field = fields[parseInt(index)]; // Ajustar índice a 0-based
                let value;
                try {
                    switch (field) {
                        case 'fechaCX':
    ...
    function setupColumnFilters() {
        ...
        filterIcons.forEach(icon => {
            icon.removeEventListener('click', handleFilterIconClick); // Evitar múltiples listeners
            icon.addEventListener('click', handleFilterIconClick);
        });

        function handleFilterIconClick(e) {
            e.stopPropagation();
            const columnIndex = parseInt(e.target.getAttribute('data-column')) - 1; // Ajustar a 0-based
            const th = e.target.parentElement;
            const existingContainer = document.querySelector('.filter-container');

            // Cerrar cualquier contenedor de filtro abierto
            if (existingContainer) {
                closeAllFilters();
            }

            // Abrir un nuevo contenedor de filtro
            ...
            clearButton.disabled = !columnFilters[columnIndex]; // Deshabilitar si no hay filtro
            ...
            input.addEventListener('input', () => {
                ...
                timeout = setTimeout(() => {
                    ...
                    clearButton.disabled = !columnFilters[columnIndex]; // Actualizar estado del botón
                }, 300);
            });
        ...
        document.removeEventListener('click', handleOutsideClick); // Evitar múltiples listeners
    ...
    function setupResizeHandles() {
        ...
        thElements.forEach((th, index) => {
            ...
            resizeHandle.addEventListener('mousedown', (e) => {
                ...
                // Guardar los anchos actuales de todas las columnas
                ...
                const onMouseMove = (e) => {
                    ...
                    // Actualizar solo la columna actual
                    ...
                    // Mantener los anchos de las otras columnas
                    ...
                    // No permitir que la tabla sea más pequeña que el ancho inicial
