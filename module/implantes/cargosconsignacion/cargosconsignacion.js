...
try {
    ...
    function applyQuickFilters(data) {
        return data.filter(item => {
            if (!item.fechaTraspaso) {
                console.warn('Documento sin fechaTraspaso:', item.docId, item);
                return true;
            }
            let fechaTraspaso;
            ...
            if (!fechaTraspaso || isNaN(fechaTraspaso)) {
                console.warn('Documento con fechaTraspaso inválida:', item.docId, item);
                return true;
            }
    ...
    async function loadCargos() {
        try {
            ...
            console.error('Error al cargar cargos:', error.code, error.message);
