...
try {
    ...
    function renderImplantes(implantes) {
        ...
        if (implantes.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="14" class="text-center">No hay datos para mostrar. Seleccione un mes para ver los implantes.</td></tr>';
            return;
        }
    ...
    function initializeColumnWidths() {
        const initialWidths = [
            '30px', // Checkbox column
            '80px', '80px', '100px', '80px', '120px', '120px', '80px', '100px', '100px', '80px', '80px', '80px', '80px'
        ];
        const headers = document.querySelectorAll('#implantes-table th');
    ...
    async function editImplante(id) {
        ...
        try {
            ...
            document.getElementById('edit-fecha-ingreso').value = formatDateInput(implante.fechaIngreso);
            document.getElementById('edit-prevision').value = implante.prevision || '';
            document.getElementById('edit-admisión').value = implante.admision || '';
            document.getElementById('edit-nombrePaciente').value = implante.nombrePaciente || '';
            document.getElementById('edit-medico').value = implante.medico || '';
            document.getElementById('edit-medico').dataset.id = implante.medicoId || '';
            document.getElementById('edit-fechaCX').value = formatDateInput(implante.fechaCX);
            document.getElementById('edit-proveedor').value = implante.proveedor || '';
            document.getElementById('edit-proveedor').dataset.id = implante.proveedorId || '';
            document.getElementById('edit-estado').value = implante.estado || 'Agendando';
            document.getElementById('edit-fecha-cargo').value = formatDateInput(implante.fechaCargo);
            document.getElementById('edit-informe').value = implante.informe || '';
            document.getElementById('edit-total-cotizacion').value = formatNumberWithThousands(implante.totalCotizacion);
    ...
    async function saveEditImplante() {
        ...
        if (!document.getElementById('edit-informe').value) {
            showMessage('El campo Informe es requerido. Por favor, seleccione una opción.', 'error');
            document.getElementById('edit-informe').focus();
            return;
        }
        const totalCotizacionValue = parseNumberWithThousands(document.getElementById('edit-total-cotizacion').value);
        if (isNaN(totalCotizacionValue)) {
            showMessage('El campo Total Cotización debe ser un número entero válido.', 'error');
            document.getElementById('edit-total-cotizacion').focus();
            return;
        }
        await showLoadingModal(true);
        try {
            ...
            const implanteData = {
                fechaIngreso: parseDateInput(document.getElementById('edit-fecha-ingreso').value),
                atributo: document.getElementById('edit-atributo').value || 'Implantes',
                prevision: document.getElementById('edit-prevision').value.trim() || '',
                admision: document.getElementById('edit-admisión').value.trim() || '',
                nombrePaciente: document.getElementById('edit-nombrePaciente').value.trim() || '',
                medico: document.getElementById('edit-medico').value.trim() || '',
                medicoId: document.getElementById('edit-medico').dataset.id || '',
                fechaCX: parseDateInput(document.getElementById('edit-fechaCX').value),
                proveedor: document.getElementById('edit-proveedor').value.trim() || '',
                proveedorId: document.getElementById('edit-proveedor').dataset.id || '',
                estado: document.getElementById('edit-estado').value || 'Agendando',
                fechaCargo: parseDateInput(document.getElementById('edit-fecha-cargo').value),
    ...
    function setupAdmisionAutocomplete() {
        if (!admisionInput || !previsionInput || !medicoInput) return;
        admisionInput.addEventListener('input', async () => {
            ...
            if (admisionValue) {
                ...
                try {
                    const reporte = await loadReporteByAdmision(admisionValue, user);
                    if (reporte) {
    ...
    function setupEditAdmisionAutocomplete() {
        if (!editAdmisionInput || !editPrevisionInput || !editMedicoInput) return;
    ...
    function clearForm() {
        ...
        delete proveedorInput.dataset.id;
        proveedorList.innerHTML = '';
        proveedorList.style.display = 'none';
        fechaCargoInput.value = '';
        informeSelect.value = '';
