...
async function checkDuplicate(nombrePaquete, excludeDocId = null) {
...
async function getNextPaqueteId() {
  if (lastPaqueteId > 0) {
    lastPa++;
    return lastPa.toString().padStart(1, '0');
  }
  const paquetesCollection = collection(db, 'paquetes');
    ...
    try {
      ...
      registrarBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          const duplicateCheck = await checkDuplicate(nombrePaquete);
          if (duplicateCheck.isDuplicate) {
            showSuccessMessage(`Error: Ya existe un paquete con el nombre "${duplicateCheck.value}"`, false);
      ...
      saveEditBtn.addEventListener('click', async () => {
        ...
        if (!nombrePaquete || !proveedor || items.length === 0 || precioTotal < 0) {
          showSuccessMessage('Por favor, completa todos los campos correctamente', false);
          return;
        }

        try {
          const duplicateCheck = await checkDuplicate(nombrePaquete, currentEditId);
          if (duplicateCheck.isDuplicate) {
            showSuccessMessage(`Error: Ya existe un paquete con el nombre "${duplicateCheck.value}"`, false);
            return;
          }
