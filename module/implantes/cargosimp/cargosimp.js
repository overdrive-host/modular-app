...
try {
        ...
        if (downloadType === 'year') {
            filteredCargos = allCargos.filter(cargo => {
                if (!cargo.fechaCx || !(cargo.fechaCx instanceof Timestamp)) {
                    console.warn(`Cargo con ID ${cargo.id} tiene fechaCx inválida: ${cargo.fechaCx}`);
                    return false;
                }
    ...
    function downloadCSV(data, filename) {
        ...
        const columns = [
            ...
            { key: 'fechaCarga', header: 'Fecha Carga' },
        ...
        mainCargos.sort((a, b) => {
            const dateA = a.fechaCarga ? a.fechaCarga.toMillis() : 0;
            const dateB = b.fechaCarga ? b.fechaCarga.toMillis() : 0;
            return dateB - dateA;
        });
    ...
    function showOcModal(cargoIds) {
        ...
        selectedCargos.forEach(cargo => {
            rows.push({
                ...
                fechaCargo: formatDateForDisplay(cargo.fechaCx),
    ...
    function renderTabla1(cargos) {
        ...
        const paginatedCargos = cargos.slice(start, end).filter(cargo => !cargo.parentCargoId);
            ...
            icon.addEventListener('click', async () => {
                ...
                if (isVisible) {
                    ...
                    subCargos.forEach(subCargo => {
                        ...
                        <td class="fecha-carga">${formatDateForDisplay(subCargo.fechaCarga)}</td>
                        <td class="acciones">
