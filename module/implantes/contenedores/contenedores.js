...
// Cargar jsPDF y autoTable al iniciar con verificación
async function loadLibraries() {
  try {
    ...
    console.log('Librerías jsPDF y autoTable verificadas exitosamente.');
    return true;
  } catch (error) {
...
function addItemRow(item = { referencia: '', descripcion: '', ideal: '', cantidad: '', precio: '' }) {
  ...
  if (precioInput) {
    precioInput.addEventListener('input', updatePrecioTotal);
  } else {
  ...
  Object.keys(filters).forEach(column => {
    if (filters[column]) {
      filteredContenedores = filteredContenedores.filter(contenedor => {
        const value = contenedor[column]?.toString()?.toLowerCase() || '';
