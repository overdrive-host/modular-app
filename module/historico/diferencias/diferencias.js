...
try {
    ...
    const elements = {
        ...
        loadingModal, loadingProgress, yearFilter, monthFilter, ocMontoTotal, providerButtonsContainer, empresaRutDisplay
    };
    ...
    function createProviderButtons(providers) {
        if (!providerButtonsContainer) return;
        providerButtonsContainer.innerHTML = '';
        const allButton = document.createElement('button');
        allButton.textContent = 'Todos los Proveedores';
        allButton.className = 'provider-btn';
        allButton.addEventListener('click', () => {
            providerFilters = {};
            applyFilters();
            displayEmpresaRut(null);
            allButton.classList.add('active');
            document.querySelectorAll('.provider-btn').forEach(btn => {
                if (btn !== allButton) btn.classList.remove('active');
            });
        });
        providerButtonsContainer.appendChild(allButton);

        providers.forEach(provider => {
            const button = document.createElement('button');
            button.textContent = provider || 'Sin Proveedor';
            button.className = 'provider-btn';
            button.addEventListener('click', async () => {
                providerFilters = { proveedor: provider };
                applyFilters();
                await displayEmpresaRut(provider);
    ...
    function applyFilters() {
        ...
        filteredRegistros = registros.filter(registro => {
            ...
            if (providerFilters.proveedor) {
                providerMatch = (registro.proveedor || '').toString().trim() === providerFilters.proveedor;
            }
    ...
    async function loadRegistros() {
        try {
            ...
            createProviderButtons(Array.from(providers));
